...
import { useGetCourseByNameQuery } from "../Redux/course";
...
const Course = () => {
    ...
    const { data, error, isLoading } =
        useGetCourseByNameQuery("courses?populate=*");
                            ...
                            {data.data.map((course) => {
                                return (
                                    <Card
                                        key={course.id}
                                        id={course.id}
                                        img={`http://localhost:1337${course.attributes.courseCoverIMG.data.attributes.url}`}
                                        class="card"
